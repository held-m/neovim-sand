...
local config = require('sand.config')
local sand = {}
...
--- Apply user settings.
---@param values table
function sand.setup(values) setmetatable(config, { __index = vim.tbl_extend('force', config.defaults, values) }) end

--- Set the colorscheme.
function sand.colorscheme()
  if vim.version().minor < 7 then
    vim.notify('Neovim 0.7+ is required for sand colorscheme', vim.log.levels.ERROR, { title = 'Sand colorscheme' })
    return
  end
